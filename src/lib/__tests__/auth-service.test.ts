...
const { loginUser, registerUser } = await import('../auth-service');
const { ConflictError, UnauthorizedError } = await import('../errors');
...
describe('auth service', () => {
  ...
  it('rejects registration when the email is already taken', async () => {
    ...
    await expect(registerUser('ada@example.com', 'password123', 'Ada')).rejects.toBeInstanceOf(ConflictError);
  });
  ...
  it('rejects login with the wrong password', async () => {
    ...
    await expect(loginUser('ada@example.com', 'wrong-password')).rejects.toBeInstanceOf(UnauthorizedError);
  });

  it('rejects login for an unknown email', async () => {
    vi.mocked(prisma.user.findUnique).mockResolvedValue(null);

    await expect(loginUser('nobody@example.com', 'password123')).rejects.toBeInstanceOf(UnauthorizedError);
  });
});
